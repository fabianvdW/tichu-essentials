...
class CardNet(nn.Module):
    ...
    def forward(self, x):
        ...
        regular_cards = x[:, :, 1:].float()  # [batch_size, 4, 13]
        special_cards = x[:, :, 0].float()  # [batch_size, 4]
        ...
        special_features = self.special_dense(special_cards).relu()
...
def predict(model, X):
    ...
    X = torch.from_numpy(X).view(-1, 4, 14).to(device)
