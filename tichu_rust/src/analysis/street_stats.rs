...
use crate::tichu_hand::{Hand, HandType, TichuHand, ACE, EIGHT, FIVE, FOUR, JACK, KING, MASK_ACES, NINE, PHOENIX, QUEEN, SEVEN, SIX, TEN, TRICK_STREET5};
...
pub fn evaluate_streets_in_play(db: &DataBase){
    ...
    for game in db.games.iter(){
        for (_, round_log) in game.rounds.iter(){
            let mut s_played = [false; 4];
            let mut iter = round_log.iter();
            while let Some(trick) = iter.next_trick(){
                if trick.trick_type == TRICK_STREET5{
                    s_played[trick.get_starting_player() as usize] = true;
                }
            }
            rounds += 1;
            for player in 0..4{
                s5_played_by_other_3[player] += (s_played[(player+1)%4] || s_played[(player+2)%4] || s_played[(player+3)%4]) as usize;
            }
        }
    }
