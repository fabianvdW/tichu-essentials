...
pub const PLAYER_0: PlayerID = 0u8;//Team 1
pub const PLAYER_1: PlayerID = 1u8;//Team 2
pub const PLAYER_2: PlayerID = 2u8;//Team 1
pub const PLAYER_3: PlayerID = 3u8;//Team 2
...
impl PlayerRoundHand{
    ...
    pub fn final_14(&self) -> Hand{
        self.first_14 ^ hand!(self.left_out_exchange_card(), self.partner_out_exchange_card(), self.right_out_exchange_card(),
                              self.left_in_exchange_card(), self.partner_in_exchange_card(), self.right_in_exchange_card())
    }
    ...
    pub fn player_id(&self) -> PlayerID{
        ((self.extras & PLAYER_ID_MASK) >> 44) as PlayerID
    }
    ...
    pub fn round_score_relative(&self) -> (Score, Score) { //Reported relative to own team;
        let mut score_team_1: Score = 0;
        let mut score_team_2: Score = 0;
        score_team_1 += self.player_0_call() as Score * 100* {if self.player_0_rank() == RANK_1{1}else{-1}};
        score_team_1 += self.player_2_call() as Score * 100* {if self.player_2_rank() == RANK_1{1}else{-1}};
        score_team_2 += self.player_1_call() as Score * 100* {if self.player_1_rank() == RANK_1{1}else{-1}};
        score_team_2 += self.player_3_call() as Score * 100* {if self.player_3_rank() == RANK_1{1}else{-1}};
        //Double Win or Card Points
        if self.player_0_rank() + self.player_2_rank() <= RANK_1 + RANK_2{
            //Double Win for Team 1
            score_team_1 += 200;
        }else if self.player_1_rank() + self.player_3_rank() <= RANK_1 + RANK_2{
...
pub type TaggedCardIndex: u8;
pub trait TaggedCardndex_t{
    pub fn get_player(&self) -> PlayerID;
    
}
pub struct RoundLog{
    log: Vec<TaggedCardIndex>
}